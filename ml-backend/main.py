...
async def extract_text_details(classification_data: ExtractionRequest):
    from ner import extract_details_from_text
    
    text = classification_data.text
    if not text.strip():
        return ExtractionResponse(venue="", date="", time="", link="")
    
    try:
        res = extract_details_from_text(text)
        return ExtractionResponse(**res)
